...
cp $HOME/server-env-for-orca-6.1.0/compute-templates/B3LYP-acn.inp B3LYP-acn/job.inp && echo "SUCCESS! (1/5)" || echo "FAILED! Что-то пошло не так"
cp $HOME/server-env-for-orca-6.1.0/compute-templates/CAM-B3LYP-acn.inp CAM-B3LYP-acn/job.inp && echo "SUCCESS! (2/5)" || echo "FAILED! Что-то пошло не так"
cp $HOME/server-env-for-orca-6.1.0/compute-templates/B2PLYP-acn.inp B2PLYP-acn/job.inp && echo "SUCCESS! (3/5)" || echo "FAILED! Что-то пошло не так"
cp $HOME/server-env-for-orca-6.1.0/compute-templates/WPBEPP86-acn.inp WPBEPP86-acn/job.inp && echo "SUCCESS! (4/5)" || echo "FAILED! Что-то пошло не так"
cp $HOME/server-env-for-orca-6.1.0/compute-templates/SOS-PBE-QIDH-acn.inp SOS-PBE-QIDH-acn/job.inp && echo "SUCCESS! (5/5)" || echo "FAILED! Что-то пошло не так"
